...
from game_server_router import app as game_server_router
...
async def register_server(server: Server, request: Request, ):
    ...
    try:
        ...
        if server_entry:
            ...
            cursor.execute(
                "INSERT INTO `servers` (`IP`, `ApiPort`, `ram`, `cpu`, `storage`, `art`) VALUES (%s, %s, %s, %s, %s, %s)",
                (server_ip, server_port, server.ram, server.cpu, server.storage, server.art)
            )
...
@app.post("/delete-vm/")
async def delete_vm(vm_control_request: VMControlRequest):
    server_info = get_server_info_by_vm_name(vm_control_request.vm_name, vm_control_request.token)
    try:
        response = post(
            f"http://{server_info['IP']}:{server_info['ApiPort']}/delete-vm/",
            json={
                "vm_name": vm_control_request.vm_name,
                "token": vm_control_request.token
            }
...
async def delete_vm(vm_control_request: VMControlRequest):
    ...
    try:
        response = post(
            f"http://{server_info['IP']}:{server_info['ApiPort']}/disable-vm/",
            json={
                "vm_name": vm_control_request.vm_name,
                "token": vm_control_request.token
            }
